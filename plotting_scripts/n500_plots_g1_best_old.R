...
p <- ggplot(momidat_basic, aes(x=data, y=NeHrat, group=sampn,color=sampn)) + geom_violin()
p + geom_point(shape=16,size=0.5) +
#  scale_color_manual(values=c("10K RAD loci"="purple","50K RAD loci"="cornflowerblue","WGS"="darkblue"))+
  ylab(expression(paste(hat(N)["E,H"],'/N'["E,H"]))) +
  geom_hline(yintercept=1,linetype="dashed") +
...
p + geom_point(shape=16,col=gonedat_basic$col) + xlab("Sample Size") + ylab(expression(paste(widehat('N'[E]),'H/N'[E],'H'))) + geom_hline(yintercept=1,linetype="dashed") + facet_wrap(~scen) + scale_y_log10() + theme(strip.text = element_text(size = 6),axis.text.x = element_text(angle = 30)) + ggtitle ("GONE")

ggsave("plots/GONE_schemes_NeH_violin.pdf",device="pdf")


p <- ggplot(gonedat_basic, aes(x=sampn, y=NeCrat)) + geom_violin()
p + geom_point(shape=16,col=gonedat_basic$col) + xlab("Sample Size") + ylab(expression(paste(widehat('N'[E]),'C/N'[E],'C'))) + geom_hline(yintercept=1,linetype="dashed") + facet_wrap(~scen) + scale_y_log10() + theme(strip.text = element_text(size = 6),axis.text.x = element_text(angle = 30)) + ggtitle ("GONE")

ggsave("plots/GONE_schemes_NeC_violin.pdf",device="pdf")
...
p + geom_point(shape=16,col=stairdat_basic$col) + xlab("Sample Size") + ylab(expression(paste(widehat('N'[E]),'H/N'[E],'H'))) + geom_hline(yintercept=1,linetype="dashed") + facet_wrap(~scen) + scale_y_log10() + theme(strip.text = element_text(size = 6),axis.text.x = element_text(angle = 30)) + ggtitle ("Stairway plot")

ggsave("plots/stairway_schemes_NeH_violin.pdf",device="pdf")


p <- ggplot(stairdat_basic, aes(x=sampn, y=NeCrat)) + geom_violin()
p + geom_point(shape=16,col=stairdat_basic$col) + xlab("Sample Size") + ylab(expression(paste(widehat('N'[E]),'C/N'[E],'C'))) + geom_hline(yintercept=1,linetype="dashed") + facet_wrap(~scen) + scale_y_log10() + theme(strip.text = element_text(size = 6),axis.text.x = element_text(angle = 30)) + ggtitle ("Stairway plot")

ggsave("plots/stairway_schemes_NeC_violin.pdf",device="pdf")
