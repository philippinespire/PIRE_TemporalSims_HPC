...
ggsave("plots/maf_NeH_violins_revised0509.pdf",device="pdf")
...
ggsave("plots/maf_NeC_violins_revised0509.pdf",device="pdf")
...
maf1err %>%
  filter(scheme=="Two Samples") -> maf12samp
...
singleerr$scen=gsub("n10e4 td190 l95","Fast Decline",singleerr$scen)
singleerr$scheme=gsub("t0t120","Two Samples",singleerr$scheme)
singleerr$maf=gsub("e0.0001","Singleton Rate 1e-4",singleerr$maf)
singleerr$maf=gsub("e0.001","Singleton Rate 1e-3",singleerr$maf)
singleerr$maf=gsub("e1e-05","Singleton Rate 1e-5",singleerr$maf)


mafcomp=rbind(nomaf2samp,maf12samp,singleerr)

mafcomp$maf=factor(mafcomp$maf,levels=c("Singleton Rate 1e-3","Singleton Rate 1e-4","Singleton Rate 1e-5","No Minor Allele Filter","Minor Allele Filter"))
...
mafcomp1$metric="NEH"
mafcomp2$metric="NEC"
mafcomp1$Nerat=mafcomp1$NeHrat
mafcomp2$Nerat=mafcomp2$NeCrat

mafcomp12=rbind(mafcomp1,mafcomp2)

mafcomp12$metric=factor(mafcomp12$metric,levels=c("NEH","NEC"))

p <- ggplot(mafcomp12,aes(y=Nerat, x=maf, fill=metric, colour=scheme)) 
p+ geom_hline(yintercept=1,linetype="dashed",col="gray") + geom_violin(position=position_dodge(width=0.5),scale="width", width=0.3, alpha=0.8,trim="false") +
  scale_color_manual(values=c("Two Samples"="black"))+
...
ggsave("plots/singlemaf_NeHCrat_050922.pdf",device="pdf")



#### maf across sampling schemes
...
p <- ggplot(mafcomp12,aes(y=Nerat, x=maf, fill=metric, colour=scheme)) 
p+ geom_hline(yintercept=1,linetype="dashed",col="gray") + geom_violin(position=position_dodge(width=0.5),scale="width", width=0.3, alpha=0.8,trim="false") +
  scale_color_manual(values=c("Two Samples"="black","Contemporary"="black","Serial Sampling"="black"))+
  scale_fill_manual(values=c("NEH"="black","NEC"="white"))+
  ylab(expression(paste(hat(N)["E"],'/N'["E"]))) +
  xlab("") +
  facet_wrap(~scheme) +
...
ggsave("plots/maf_schemes_NeHCrat_050922.pdf",device="pdf")
