...
it=2
	...
	data=wgs_n50
	lgm=c
	first=${dir}${sim}expansion_recap/vcf/i${it}_chr_1_${data}_contemp_${maf}.recode.vcf.gz
	outfile=${dir}${sim}expansion_recap/vcf/i${it}_allchr_${data}_contemp_${maf}.recode.vcf
	vcfdir=${dir}${sim}expansion_recap/vcf/
	outt0=${dir}${sim}expansion_recap/vcf/i${it}_allchr_${data}_${lgm}_${maf}_t0
	outplink=${dir}${sim}${data}_i${it}/GONE_exp/i${it}_allchr_${data}_${lgm}_${maf}
	goneinput=i${it}_allchr_${data}_${lgm}_${maf}
	mkdir -p ${dir}${sim}${data}_i${it}/GONE_exp
	cp -r /home/br450/GONE/Linux/* ${dir}${sim}${data}_i${it}/GONE_exp
	zcat < ${first} | grep '^##fileformat' > ${outfile}
	zcat < ${first} | grep '^##source' >> ${outfile}
	zcat < ${first} | grep '^##FILTER' >> ${outfile}
	for chr in {1..25}
	do
		chrfile=${dir}${sim}expansion_recap/vcf/i${it}_chr_${chr}_${data}_contemp_${maf}.recode.vcf.gz
	...
	cd ${dir}${sim}${data}_i${it}/GONE_exp
