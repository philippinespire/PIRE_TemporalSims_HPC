...
def allele_counts(i,c):
	...
	sfs=momi.SnpAlleleCounts.read_vcf(vcf_file=vcffile,ind2pop=ind2popdict,bed_file=bedfile,ancestral_alleles=False)
	sfs.dump(outfile)
	vcffile = outdir + outsim + "constant_recap/vcf/i" + str(i) + "_chr_" + str(c) + "_rad10k_n500_" + scheme + "_nomaf.recode.vcf.gz"
	ind2popfile = outdir + outsim + "i" + str(i) + "_n500_ind2pop_" + scheme + ".txt"
	bedfile = dir + sim + "i" + str(i) + "_chr_" + str(c) + "_bed10k.bed"
	outfile = outdir + outsim + "constant_recap/vcf/i" + str(i) + "_n500_rad10k_chr" + str(c) + "_" + scheme + "_nomaf.snpAlleleCounts.gz"
	with open(ind2popfile) as f:
		ind2popdict = dict([l.split() for l in f])
	sfs=momi.SnpAlleleCounts.read_vcf(vcf_file=vcffile,ind2pop=ind2popdict,bed_file=bedfile,ancestral_alleles=False)
	sfs.dump(outfile)
	vcffile = outdir + outsim + "constant_recap/vcf/i" + str(i) + "_chr_" + str(c) + "_rad50k_n500_" + scheme + "_nomaf.recode.vcf.gz"
	ind2popfile = outdir + outsim + "i" + str(i) + "_n500_ind2pop_" + scheme + ".txt"
	bedfile = dir + sim + "i" + str(i) + "_chr_" + str(c) + "_bed50k.bed"
	outfile = outdir + outsim + "constant_recap/vcf/i" + str(i) + "_n500_rad50k_chr" + str(c) + "_" + scheme + "_nomaf.snpAlleleCounts.gz"
	with open(ind2popfile) as f:
		ind2popdict = dict([l.split() for l in f])
	sfs=momi.SnpAlleleCounts.read_vcf(vcf_file=vcffile,ind2pop=ind2popdict,bed_file=bedfile,ancestral_alleles=False)
	sfs.dump(outfile)
	...
	sfs=momi.SnpAlleleCounts.read_vcf(vcf_file=vcffile,ind2pop=ind2popdict,bed_file=bedfile,ancestral_alleles=False)
	sfs.dump(outfile)
	vcffile = outdir + outsim + "constant_recap/vcf/i" + str(i) + "_chr_" + str(c) + "_rad10k_n500_" + scheme + "_maf1.recode.vcf.gz"
	ind2popfile = outdir + outsim + "i" + str(i) + "_n500_ind2pop_" + scheme + ".txt"
	bedfile = dir + sim + "i" + str(i) + "_chr_" + str(c) + "_bed10k.bed"
	outfile = outdir + outsim + "constant_recap/vcf/i" + str(i) + "_n500_rad10k_chr" + str(c) + "_" + scheme + "_maf1.snpAlleleCounts.gz"
	with open(ind2popfile) as f:
		ind2popdict = dict([l.split() for l in f])
	sfs=momi.SnpAlleleCounts.read_vcf(vcf_file=vcffile,ind2pop=ind2popdict,bed_file=bedfile,ancestral_alleles=False)
	sfs.dump(outfile)
	vcffile = outdir + outsim + "constant_recap/vcf/i" + str(i) + "_chr_" + str(c) + "_rad50k_n500_" + scheme + "_maf1.recode.vcf.gz"
	ind2popfile = outdir + outsim + "i" + str(i) + "_n500_ind2pop_" + scheme + ".txt"
	bedfile = dir + sim + "i" + str(i) + "_chr_" + str(c) + "_bed50k.bed"
	outfile = outdir + outsim + "constant_recap/vcf/i" + str(i) + "_n500_rad50k_chr" + str(c) + "_" + scheme + "_maf1.snpAlleleCounts.gz"
	with open(ind2popfile) as f:
		ind2popdict = dict([l.split() for l in f])
	sfs=momi.SnpAlleleCounts.read_vcf(vcf_file=vcffile,ind2pop=ind2popdict,bed_file=bedfile,ancestral_alleles=False)
	sfs.dump(outfile)
